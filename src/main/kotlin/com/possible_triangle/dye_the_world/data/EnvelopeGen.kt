...
fun <T : Block, P> BlockBuilder<T, P>.envelopeShaftBlockstate() =
...
fun <T : Block, P> BlockBuilder<T, P>.envelopeBlockstate() =
    blockstate { context, provider ->
        val model = provider.models().cubeAll(name, texture)
        provider.simpleBlock(context.get(), model)
    }

fun <T : Item, P> ItemBuilder<T, P>.envelopeShaftItemModel() =
...
fun <T : Item, P> ItemBuilder<T, P>.envelopeRecipe() =
    recipe { context, provider ->
        ...
        ItemApplicationRecipe
            .Builder(::DeployerApplicationRecipe, context.id)
