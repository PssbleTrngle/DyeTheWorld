...
fun <T : Block, P> BlockBuilder<T, P>.buntingBlockState() = blockstate { context, provider ->
...
fun <T : Block, P> BlockBuilder<T, P>.wallBuntingBlockState() = blockstate { context, provider ->
...
fun <T : Item, P> ItemBuilder<T, P>.dyedBuntingRecipe() = recipe { context, provider ->
    ...
    provider.dyeingRecipe(dye, ModRegistry.BUNTING_BLOCKS[DyeColor.WHITE]!!.get(), context, id = Constants.MOD_ID.createId("bunting_${dye}_dyeing")) {
        group("bunting")
    }
}