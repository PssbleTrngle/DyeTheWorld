...
fun <T : Item, P> ItemBuilder<T, P>.glassDoorRecipes() = recipe { context, provider ->
    provider.door(
...
fun <T : DoorBlock, P> BlockBuilder<T, P>.glassDoorBlockstate() = blockstate { context, provider ->
...
fun <T : Block, P> BlockBuilder<T, P>.glassDoorLoot() = loot { provider, block ->
    provider.dropWhenSilkTouch(block)
}

fun <T : Item, P> ItemBuilder<T, P>.glassDoorItemModel() = model { context, provider ->
    val texture = MOD_ID.createId("item/$CLAYWORKS/${dye}_stained_glass_door")
    provider.generated(context, texture).translucent()
}

fun <T : Item, P> ItemBuilder<T, P>.glassTrapdoorRecipes() = recipe { context, provider ->
    provider.square(
        DataIngredient.items(dye.itemOf("stained_glass")),
        RecipeCategory.REDSTONE,
        context,
...
fun <T : TrapDoorBlock, P> BlockBuilder<T, P>.glassTrapdoorBlockstate() = blockstate { context, provider ->
...
fun <T : Block, P> BlockBuilder<T, P>.glassTrapdoorLoot() = loot { provider, block ->
    provider.dropWhenSilkTouch(block)
}

fun <T : Item, P> ItemBuilder<T, P>.glassTrapdoorItemModel() = model { context, provider ->
